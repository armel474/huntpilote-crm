/**
 * Tests des contrats des actions serveur de l'Agence hub, avec un client de
 * requête simulé : aucune base contactée. Repris de la PR nº 3
 * (`codex/agence-hub-socle`) et adaptés aux chemins et contrats de cette
 * branche (`app/agence/actions.ts`).
 *
 *   node --test tests/
 */
...
function setup({ configured = true, session = { kind: 'membre', memberId: 'self', agencyId: 'agency', role: 'admin' }, rows = [{ id: 'self', role: 'admin' }], rolePermissions = [{ permission: 'manage_team' }], overrides = [], error = null } = {}) {
  ...
  const actions = loadTs('app/agence/actions.ts', {
...
test('La sauvegarde du profil cible son agence et actualise le hub et les anciens accès', async () => {
  ...
  assert.equal(JSON.stringify(s.invalidations), JSON.stringify([['/agence'], ['/parametres']]));
});
...
test('Un membre sans gestion ne déclenche aucune écriture de droits, même en envoyant des permissions', async () => {
  const s = setup({ session: { kind: 'membre', memberId: 'self', agencyId: 'agency', role: 'redacteur' } });
  const result = await s.actions.updateMember(null, form({ member_id: 'self', first_name: 'Test', permission: 'manage_team', active: '0' }));
  assert.equal(result.ok, true);
  const payload = s.calls.find((c) => c.method === 'update').args[0];
  assert.equal('active' in payload, false);
  assert.equal(s.calls.some((c) => c.table === 'member_permission' || c.table === 'role_permission'), false);
});

test('Le retrait de sa gestion d’équipe est refusé avant même la vérification du rôle', async () => {
  const s = setup({ rolePermissions: [] });
  const result = await s.actions.updateMember(null, form({ member_id: 'self', first_name: 'Test', manage: '1', active: '1', role: 'admin' }));
  assert.equal(result.ok, false);
  assert.equal(s.calls.length, 0);
});
