...
export async function updateMember(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  // Refuser avant toute écriture, y compris de l'identité et du rôle.
  if (managing && memberId === session.memberId && !wanted.has('manage_team')) {
    return fail('Vous ne pouvez pas vous retirer la gestion de l’équipe.');
  }
  let engagement = {};
  if (managing) {
    const role = text(fd, 'role') as Role | null;
    if (!role || !ROLES.includes(role)) return fail('Choisissez un rôle.');
    if (memberId === session.memberId && role !== session.role) {
      // Le rôle est écrit avant les exceptions nominatives. Ne pas perdre
      // manage_team entre ces deux requêtes : RLS empêcherait de le rétablir.
      const [defaults, override] = await Promise.all([
        ...
        supabase.from('member_permission').select('granted').eq('member_id', memberId).eq('permission', 'manage_team').eq('granted', true),
      ]);
      ...
      if (!defaults.data?.length && !override.data?.length) {
        return fail('Ce rôle retirerait votre gestion de l’équipe avant l’enregistrement des droits. Demandez à une autre personne qui gère l’équipe de modifier votre rôle.');
      }
    }
