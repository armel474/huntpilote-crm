...
## Frontière avec l'écran d'audit déjà livré

L'écran **détail d'audit** (`/clients/[id]/audits/[a]`) existe. Il montre un
score pondéré sur trois dimensions, relevé une fois par mois. Site Audit est
l'outil qui **alimente** la dimension SEO — ce n'est pas le même écran et il ne
doit pas en reproduire le sommet.

| | Détail d'audit | Site Audit (cet outil) |
|---|---|---|
| Rythme | Mensuel, automatique | À la demande |
| Portée | Trois dimensions pondérées | Le crawl seul |
| Ce qu'on y lit | Un score et son évolution | Les pages, une par une |
| Ce qu'on y fait | Décider quoi traiter | Trouver et délimiter |

**N'affiche pas de score global pondéré ici** : il appartient à l'audit. En
revanche, la ligne de constat est la même — `.crit`, avec sa mesure, son seuil,
son icône de statut et son bouton « créer la priorité ». Reprends-la telle
quelle.
