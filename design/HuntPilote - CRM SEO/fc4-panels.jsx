...
const { Ico, Badge, Dot, Gauge, Spark, Lbl, FC3_CLIENT: C4, FC3_KPIS, FC3_PRIORITIES, FC3_TASKS, FC3_PROOFS, FC3_COMPETITORS, FC3_MY, FC3_ACTIVITY, FC3_SPARK } = window;
...
const V4_COMMS = [
  {type:'Email',tone:'blue',subject:'Rapport SEO · Avril 2026 envoyé',who:'Automatique',date:'2 mai 2026',state:'Ouvert le 2 mai'},
  {type:'Appel',tone:'green',subject:'Point mensuel — priorités T2',who:'Marie Chen',date:'28 avr. 2026',state:'32 min'},
  {type:'Email',tone:'blue',subject:'Validation du plan éditorial (5 articles)',who:'Marie Chen',date:'21 avr. 2026',state:'Répondu'},
  {type:'Réunion',tone:'violet',subject:'Restitution Audit Complet Q2',who:'Marie Chen + client',date:'16 avr. 2026',state:'Visio · 45 min'},
  {type:'Email',tone:'blue',subject:'Rapport SEO · Mars 2026 envoyé',who:'Automatique',date:'3 avr. 2026',state:'Ouvert le 4 avr.'}];
...
const LeftPanel = ({ uxState, setTab }) => {
  ...
  return (
    <div style={{width:'var(--left-w)',flexShrink:0,borderRight:'1px solid var(--bd)',background:'var(--bg-1)',display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      <div style={{flex:1,overflowY:'auto',padding:'18px 14px 14px'}}>
        ...
        <Lbl text="Contact"/>
        {[[Ico.Mail,C4.email],[Ico.Phone,C4.phone],[Ico.Globe,C4.website],[Ico.Pin,C4.address]].map(([Icon,val],i)=>(
          <div key={i} style={{display:'flex',alignItems:'center',gap:7,marginBottom:7,color:'var(--fg-2)'}}>
            ...
            <span style={{fontSize:'0.6875rem',minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{val}</span>
          </div>))}
        {rule}
...
const PanelReports = () => (
  <div style={{display:'flex',flexDirection:'column',gap:10,paddingBottom:16}}>
    ...
    <div style={{...c4,padding:'14px 16px'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
        <div style={{fontSize:'0.8125rem',fontWeight:800,color:'var(--fg-1)'}}>Historique des communications</div>
        <button style={b4Out}><Ico.Plus/>Consigner un échange</button>
      </div>
      <div className="fc4-table" style={{display:'grid',gridTemplateColumns:'76px 1fr 130px 100px 110px',gap:'0 12px',alignItems:'center'}}>
        {['Type','Objet','Par','Date','Statut'].map(h=><div key={h} style={{fontSize:'0.5rem',fontWeight:700,color:'var(--fg-4)',textTransform:'uppercase',letterSpacing:'0.1em',paddingBottom:7,borderBottom:'1px solid var(--bd-1)'}}>{h}</div>)}
        {V4_COMMS.map((m,i)=>(
          <React.Fragment key={i}>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)'}}><Badge label={m.type} tone={m.tone} sm/></div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.75rem',fontWeight:600,color:'var(--fg-1)',minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.subject}</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.6875rem',color:'var(--fg-2)'}}>{m.who}</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.6875rem',color:'var(--fg-3)',fontVariantNumeric:'tabular-nums'}}>{m.date}</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.625rem',color:'var(--fg-3)'}}>{m.state}</div>
          </React.Fragment>))}
      </div>
    </div>
  </div>
