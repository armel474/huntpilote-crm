...
/* ── PANEL CONTRAT ── */
const PanelContrat = ({ contacts, onOpenContact, setTab }) => {
  const [quotes, setQuotes] = usePanelState(() => window.QT_QUOTES.map(q => ({ ...q, lignes: q.lignes.map(l => ({ ...l })), versions: q.versions.map(v => ({ ...v })) })));
  const [view, setView] = usePanelState(null); // null | {id} | 'new' | {edit:id}
  const [genPanelOpen, setGenPanelOpen] = usePanelState(false);
  const activeContacts = (contacts || []).filter(c => c.status === 'actif');
  const closeView = () => setView(null);

  const saveNewQuote = data => { setQuotes(qs => [{ id: 'DV-2026-0' + (30 - qs.length), statut:'brouillon', emis:null, versions:[], ...data }, ...qs]); closeView(); };
  const correctQuote = (data, id) => { setQuotes(qs => qs.map(q => q.id === id ? { ...q, ...data, statut:'brouillon', emis:null } : q)); closeView(); };
  const sendQuote = id => setQuotes(qs => qs.map(q => {
    if (q.id !== id) return q;
    const nextV = q.versions.length + 1;
    return { ...q, statut:'envoye', emis: q.emis || 'aujourd’hui', versions:[...q.versions, { v:nextV, at:'aujourd’hui', who:'Vous', note: nextV === 1 ? 'Version envoyée au client par courriel.' : `Version ${nextV} envoyée au client — correction de la version ${nextV - 1}.` }] };
  }));
  const requestCorrection = id => setView({ edit:id });

  const currentQuote = view && view.id ? quotes.find(q => q.id === view.id) : null;
  const editQuote = view && view.edit ? quotes.find(q => q.id === view.edit) : null;
  ...
  return (
    ...
    <div style={{...card,padding:'14px 16px',marginBottom:10}}>
      ...
      {quotes.some(q=>q.statut==='accepte') && (
        <div style={{marginTop:10,paddingTop:10,borderTop:'1px solid var(--bd)',display:'flex',flexDirection:'column',gap:5}}>
          {quotes.filter(q=>q.statut==='accepte').map(q=>(
            <div key={q.id} style={{fontSize:'0.625rem',color:'var(--fg-3)',display:'flex',alignItems:'center',gap:5}}><Ico.Check/><b style={{color:'var(--fg-1)'}}>{q.contratRef}</b> — {q.objet}</div>
          ))}
        </div>
      )}
    </div>
    <div style={{...card,padding:'14px 16px',marginBottom:10}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
        <Lbl text="Historique des factures" mb={0}/>
        <span style={{fontSize:'0.5625rem',color:'var(--fg-4)'}}>{window.QT_INVOICES.length} factures</span>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'110px 1fr 100px 100px 100px',gap:'0 12px'}}>
        {['Numéro','Objet','Montant','Date','Statut'].map(h=><div key={h} style={{fontSize:'0.5rem',fontWeight:700,color:'var(--fg-4)',textTransform:'uppercase',letterSpacing:'0.1em',paddingBottom:7,borderBottom:'1px solid var(--bd-1)'}}>{h}</div>)}
        {window.QT_INVOICES.map(f=>(
          <React.Fragment key={f.id}>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.6875rem',fontWeight:700}}>{f.id}</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.75rem',color:'var(--fg-1)'}}>{f.objet}</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.75rem',fontWeight:700,fontVariantNumeric:'tabular-nums'}}>{f.montant.toLocaleString('fr-CA')} $</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)',fontSize:'0.625rem',color:'var(--fg-3)'}}>{f.date}</div>
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)'}}><Badge label="Payée" tone="green" sm/></div>
          </React.Fragment>
        ))}
      </div>
    </div>

    <window.QuotesSection contacts={activeContacts} quotes={quotes} onOpen={id=>setView({id})} onNew={()=>setGenPanelOpen(true)}/>

    {genPanelOpen && (
      <window.NewDocumentPanel context={{ clientName: 'Acme Corp.', contactName: activeContacts[0] ? activeContacts[0].name : '', contactTitre: activeContacts[0] ? activeContacts[0].role : '' }}
        onClose={() => setGenPanelOpen(false)} onCreated={() => setGenPanelOpen(false)} />
    )}

    {currentQuote && (
      <window.QuoteSheet quote={currentQuote} contacts={contacts||[]} onClose={closeView} onSend={sendQuote} onNewVersion={requestCorrection}
        onGoThread={()=>{ setTab && setTab('communications'); closeView(); }} onGoContract={closeView}/>
    )}

    {(view==='new' || editQuote) && (
      <React.Fragment>
        <div className="ct-scrim" onClick={closeView}></div>
        <aside className="ct-sheet" role="dialog" aria-modal="true" aria-label={view==='new' ? 'Nouveau devis' : 'Corriger le devis'}>
          <div className="ct-head">
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:'0.9375rem',fontWeight:800,letterSpacing:'-0.02em'}}>{view==='new' ? 'Nouveau devis' : `Corriger ${editQuote.id}`}</div>
              <div style={{fontSize:'0.625rem',color:'var(--fg-3)',marginTop:2}}>Acme Corp.</div>
            </div>
            <button className="hdr-btn" onClick={closeView} aria-label="Fermer le panneau"><window.IcoX2/></button>
          </div>
          <div className="ct-body">
            <window.QuoteForm contacts={activeContacts} initial={editQuote} onCancel={closeView}
              onSave={data => editQuote ? correctQuote(data, editQuote.id) : saveNewQuote(data)}/>
          </div>
        </aside>
      </React.Fragment>
    )}
