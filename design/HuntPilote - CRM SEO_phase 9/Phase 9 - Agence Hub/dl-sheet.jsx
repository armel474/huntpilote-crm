...
const DealPanel = ({ deal, stages, owners, onClose, onWin, onLose, onLog }) => {
  ...
  const [docPanel, setDocPanel] = useDS(false);
  ...
  return (
    <React.Fragment>
      ...
      <aside className="dl-sheet" role="dialog" aria-modal="true" aria-label={`Détail du deal ${deal.company}`}>
        ...
        <div className="dl-body">
          ...
          <DSec title="Documents" sub={d.docs.length ? null : 'Aucun devis ni proposition pour l’instant.'}
            right={!isWon && !isLost && (deal.stage === 'proposition' || deal.stage === 'negociation') && <button className="btn-out" style={{ fontSize: '0.625rem' }} onClick={() => setDocPanel(true)}><DI.Plus />Nouveau document</button>}>
            {d.docs.length === 0
      ...
      {docPanel && <window.NewDocumentPanel context={{ clientName: deal.company, contactName: d.contact, contactTitre: d.role || '', objetSuggere: `Proposition — ${deal.company}` }} onClose={() => setDocPanel(false)} onCreated={() => setDocPanel(false)} />}
    </React.Fragment>);
};
